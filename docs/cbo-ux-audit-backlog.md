...
2. **E2 deferred beats store English enum ids (P1 before those beats ship, P3 today)** — *display side fixed 2026-08-06*: they still STORE machine ids, but every surface (panel, coordinator drawer, export) now renders them through the catalog, so the storage choice is no longer user-visible. Remaining: — `current_use` (encontro-2.md:307), hazard ranking ids (322-326), `land_tenure` (342), consolidated write (392); closing references `{primary_hazard_label}` that nothing produces (398). Port the E1 "store the PT chip label" rule + extend cbo-field-catalog to intervention_site enums when enabling.
3. ~~**intervention_site machine fields render as document rows (P2, S)**~~ — **FIXED 2026-08-06** (doc-tab Portuguese pass). Labels + enum values for every section now resolve through `shared/cbo-field-catalog.ts`; `_`-prefixed internals stay hidden. Original entry: — `site_lat/site_lng/site_geometry/site_deferred` show as table rows ("site geometry: POLYGON((…))"); most E2 field names have no `cbo.fields.*` locale key (bairro, current_use, primary/secondary_hazard, community_anchoring_lead, community_engagement_methods) → humanized English fallbacks. Hide machine fields from render + add labels.
...
6. ~~**Markdown export is all-English (P2, S/M)**~~ — **FIXED 2026-08-06** (same pass): the export reads the session language and the same catalog, and no longer dumps `_depth_json`/`_reco_json` into the org's own document. Original entry: — `exportCboMarkdown` headers + raw values; community-facing download button.
...
25. ✅ **[SHIPPED 2026-08-06 — inline W2 read on the card + stuck-first sorting]** **Richer organization cards in the orchestrator (P1, M)** — "progress per city". **Refined with JVP: not a new cross-city screen.** Stay on the current per-cohort board and put the signal on the cards, all four groups:
...
### Wave: Vila Flores before the 30 Sept convening — JVP, 2026-09-21

35. **✅ SHIPPED 2026-09-21 (`docs/org-profile.md`) — "Baixar perfil" — one polished PDF of everything an organisation has shared so far, at any stage (P0 for 30 Sept, M)** — asked for by Vila Flores (Antonia, 2026-09): *"ter o pdf das etapas 1 e 2 impresso para que cada organização já possa visualizar o que colocou de informação na plataforma"* and *"a clear, visually accessible PDF bringing together the information entered into the platform, which we could use during the technical visits and later as a basis for building the portfolio."* Their plan for the day: every organisation gets its Etapa 1 + 2 printed; with that in hand and Robson in the room they fill Etapa 3 together; the afternoon of the 30th and all of 1 Oct are technical visits that use the platform record as the preliminary territorial diagnosis, to be validated or adjusted on site with photographs. ⚠️ **They also asked for access to Etapa 3 by 23 Sept** to get familiar and suggest adjustments — an operational item (staging link or the production republish), not code.

    **Where:** the per-organisation drawer on the orchestrator (the header that already has the org name · Export · Refresh · Arquivos · Conversa…). Beside **Export**, a **Baixar perfil** (PDF). Same button on the organisation's own chat later; and the same document at project level once projects are in use (`docs/projects.md` — the brief and the note are the multi-org cousins of this).

    **What it is:** sort of the "nota de protocolo" — a document the organisation would hand to someone. Everything shared so far, **stage-aware**: it must work at any point. After E1 only → who they are. After E2 → plus the place (map thumbnail, tenure, use, worry, their own words, photographs, what the neighbourhood risk layers say). After E3 → plus the solutions tested and the comparison. Sections that have nothing yet are absent, never an empty heading. It is NOT the context-bundle export (that is for a model) and NOT the Resumo do Projeto (that is an argument built from E3); it is the record, laid out for a person.

    **Register:** `docs/document-register.md` applies in full — third person, sources, no design rationale on the page. "Very polished, for them to share": this is the first document many of these organisations will show a community, so typography, the map and the photographs carry it. Print-first (A4, they are printing a stack of them on the 29th), readable in greyscale.

    **Build notes:** `printShell.ts` is the skeleton; `shared/field-destiny.ts` already knows every public field, its label and which document section it feeds — the profile should be driven from that registry so a field added to an encontro appears in the profile without anyone remembering (the exact failure `field-destiny` was written for). `buildContextMarkdown` has the grouping by encontro. Map thumbnail: a static tile composite or the existing site card's render. Photographs: the uploaded images from the documents store, captioned with the pre-digested observation. A batch route for the coordinator ("baixar todos", one PDF per org or one merged) is what they will actually use on the 29th.

    **Open questions for Vila Flores:** one page or several per organisation; whether the maturity scores belong on a page the organisation sees (today they are coordinator-only); whether the technical-visit notes (Robson's "leitura técnica") should print on it or on a separate visit sheet with room to write.

    **Shipped:** drawer → *Perfil (PDF)*, roster → *Todos os perfis (PDF)*, pt/en, stage-aware, with the technical reading on the organisation's copy and no scores (JVP's calls, 21 Sept). **Left:** the same document on the organisation's own chat; a project-level profile; the risk layer drawn on the map; and the two questions still open with Vila Flores — whether the scores ever belong on a page the organisation sees, and whether the visit sheet should be its own page.

### Wave: after the 22 Sept staging run — JVP

36. **The synergy report and the test copies: who is in the room, and which row is read (P2, M)** — JVP, 22 Sept: *"the synergy report is not including these new orgs I created"*. Partly true, and the two halves have different causes.

    **Excluded by design.** Every clone made with "Criar cópia de teste" is written `excludeFromPortfolio: true` (`cohortRoutes.ts:1363`), and the synergy pass filters those out twice — once for their documents, once for the member list (`cohortRoutes.ts:656`, `:675`). That is right: a test copy must not become a programme line. But nothing says so. The roster badge reads "Fora do portfólio" and the report never states how many organisations it left out or why, so an organisation that was worked in all afternoon simply is not there. **Fix:** a line under the report's header — *"N organizações fora do portfólio não entraram nesta análise"* — and the same sentence in the drawer beside the toggle.

    **Read the row they worked in.** The 22 Sept report shows `test mobile 11 929 · Etapa 3: biovaletas`, one solution, while that afternoon's session ended with four tested and compared. Four roster cards carry that same org name (3/27, 15/27, 0/27, 3/27) and each is a different `cbo_states` row; the report picked the in-portfolio one, which is not the one the workshop happened in. Same root as the two disagreeing exports (§ the audit of 22 Sept): nothing on a card, an export or a report says **which session id** it is. **Fix:** stamp `cbo_state_id` + `updatedAt` on every artefact (report header, `perfil.json`, the markdown export), and show the session's last-activity time on the roster card so two copies of one name can be told apart at a glance.

    **Already fixed, needs a staging pull:** that PDF still prints raw ids — `heat`, `enxurrada`, `parques-e-florestas-urbanas` — in the "A REDE NUM RELANCE" table. #557 (22 Sept) replaced those with labels and added the kept / still-considering / set-aside split. The report has to be regenerated after staging pulls `main`.

    **Worth deciding with Vila Flores:** whether a coordinator should be able to include a test copy on purpose (`includeInPortfolio`) for a dry run of the portfolio conversation, or whether test copies stay out categorically.

37. **Encontro 3 audit, 22 Sept — one price per solution, the tail stays off, documents say which encontro (P0 for 30 Sept, shipped as PRs)** — [`docs/w3-audit-2026-09-22.md`](w3-audit-2026-09-22.md). Two staging records replayed through every surface with `scripts/w3-surfaces-audit.ts`. Shipped: #562 (board / hoja de ruta / Resumo priced at each test's size — was 3.5× the card for Caldas, 2 trees for 25), #563 (the tail needs its own stamp; the E3 skill and fallback prompt describe the flow the platform runs), #564 (Resumo / hoja de ruta under "Para o Encontro 4"; no second-person advisor line in the Resumo), #565 (a skip is not "nada"; a trace for a test is that test's size). **Open for a person:** maturity comparability across the roster (hide phase-3 scores until E4, or confirm the advisor's baseline draft), heat-focus shelf seating everything water, Robson's approval doors. **After 30 Sept:** the cut list in the audit — retire `buildDossier` as a second engine, delete the tail once E4 exists (~700 lines), typed map turns, one `setTestSize()`.

### Wave: Vila Flores <> PxG <> OEF <> BwB, 24 Sept — before and after the 30 Sept convening

Transcript: `knowledge/_meetings/2026-09-24-cougar-vila-flores-pxg-oef-bwb.md` (Aline, Antonia, Julia, Robson). What changed in how they see the tool: **articulators are the power users, communities are not** (Robson: *"I don't see the representants of the communities as hard users of the platform"*); the next phase builds a **pre-portfolio** — a letter of intentions for COLLECTIVE projects by risk and territory, individual projects not elevated (Julia); articulators may frame it top-down to meet November (Aline's "the glue is heat waves"); launch in December.

Shipped from this meeting: #569 (photo captions in the session's language, about the site — ⚠️ run `scripts/photo-captions.ts --dir` live on staging, then `--backfill --apply`), #570 (photo guidance: the spot where they'd act, the ground, "a couple is enough", send later from the room), #571 (closing box at the end of Etapa 3 — the visit and the room on the record), #572 (Perfil: a full page of visit notes; every label and value in Portuguese).

38. **E1 → E3 in one sitting, in the room (P0 for 30 Sept, S — verify first)** — three organisations (Estevão/Siciliano, Semente, Sandra's) start from zero at Vila Flores with Aline beside them, and will likely do Etapa 3 the same day. Walk one brand-new org through all three encontros in one session as the room will: phase unlocks, who opens each encontro on the board, photos sent later over WhatsApp never blocking. Julia: *"can you complete step one and two without having pictures?"* — the answer must be yes.
39. **Articulator-only Step 4, after Etapa 3 (P1, M)** — Julia: *"step four is with us, it's not open for them."* Visit findings, Robson's reading and the team's synthesis, kept apart from what the organisation entered. Today that is one coordinator field (`technical_note`, profile tab) — make it a real step: per org, audio / files / notes by the articulators, printed under its own heading, feeding the synergy pass as the coordination's reading.
40. **Pre-portfolio structure (P0 for November, L — needs Robson's framework first)** — collective projects grouped by **risk** (heat, water management) and **territory**, each with Robson's **readiness horizon**: *pronto agora · 2–6 meses · 1 ano* — and the **critical areas** where the network must put more support (*"if we identify some critical area and maybe to be a bottleneck or a gap"*). This is Antonia's "diagnosis" document; it follows the post-workshop synthesis meeting (Aline, Antonia, Julia, Robson). Build on Robson's framework, not before it.
41. **Solutions by risk on the map (P1, M)** — Julia's agenda for the 30th: *"how many solutions for heat we do have in which territories… and then which are the solutions for water management"*. The synergy report groups by mechanism; there is no map of tested / kept solutions by risk per territory on the coordinator side.
42. **Projects are collective by default (P1, S)** — the Projects tab accepts any grouping. Julia: *"we as a network have the responsibility of not putting any organization first in the name of the others"* — a single-organisation project needs a stated reason and lands in "revisto depois", never in the pre-portfolio.
43. **The Projects tab is for articulators (P2, M)** — communities will not run project sessions; the door, the brief and the page should be designed for Aline / Antonia / Julia / Robson iterating together, not for an organisation alone on a phone.
44. **A framing theme on the synergy report (P2, S)** — Aline's "glue": articulators set it (e.g. calor) and the report groups under it, still labelled "hipóteses para validar".
45. **Roles in the network (P2, M)** — Julia wants organisations to become coordinators of areas of the pre-portfolio; the tool could record who takes which area.

**Keep in mind (not features):** Robson's classroom — *"the students that made the homework and the students that haven't"*: roster progress and "Encontro 3 não feito" must not read as a ranking in the room. And the visits are the real test (Antonia: *"a lot of new and more substantial feedbacks"*) — plan a round of changes around 2 Oct.
