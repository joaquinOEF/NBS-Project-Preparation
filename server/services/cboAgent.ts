...
async function streamWithSdk(cboId: string, userMessage: string, state: CboState, pushEvent: EventPusher, lang: string = 'en') {
  ...
  // Decision-log window: synthesis turns (Sonnet+thinking) get 30 messages
  // so the agent can reason about E1-E4 history when scoring maturity in E5.
  ...
  console.log(`[cbo] Turn for ${cboId} phase=${state.phase} sections=${sectionsFilled}/7 model=${model ?? 'sdk-default'} thinking=${thinkingBudget} log-window=${decisionLogWindow}${isFileUpload ? ' (file-upload escalation)' : ''}`);
