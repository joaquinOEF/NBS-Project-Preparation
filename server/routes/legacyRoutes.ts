// Legacy city-prototype routes (audit DS-3) — the ~1,900 lines that used to
// live inline in server/routes.ts: CityCatalyst OAuth, sample-mode init, the
// authed city/project CRUD, on-demand geospatial fetchers, the impact-model
// LLM endpoints, and the project state machine. Registered ONLY when
// ENABLE_LEGACY_ROUTES != '0', via dynamic import (same pattern as
// testRoutes): with the flag off the routes don't exist and none of the
// legacy services load — several of them throw at import time on missing
// env (OAuth client id, OpenAI key), which is also why a bare local boot of
// the workshop app used to crash.
//
...
import { registerConceptNoteRoutes } from './conceptNoteRoutes';
...
export function registerLegacyRoutes(app: Express): void {
  ...
  // Legacy-only route files (project agent chat + concept note) ride the same
  // flag — they were previously gated individually in routes.ts.
